...
impl ConnectionPool {
    pub fn new<T>(config: PoolConfig, uri: T) -> DataBaseResult<Self>
    where
        T: Into<String>,
    {
...
impl CustomizeConnection<SqliteConnection, DataBaseError> for DatabaseCustomizer {
    fn on_acquire(&self, conn: &mut SqliteConnection) -> DataBaseResult<()> {
